...
import { Loader2, TrendingDown, ChevronLeft, ChevronRight } from "lucide-react";
...
function situacaoBadge(s?: string) {
  ...
  if (sl.includes("pago") || sl.includes("quitado")) return <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200">{s}</Badge>;
...
export default function ContasPagarPage() {
  ...
  async function load(p = 1) {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("ssotica-financeiro", {
        body: { tipo: "contas_pagar", companyId, startDate, endDate, page: p, perPage: 100 },
      });
  ...
  const filtered = rows.filter(r => {
    const nome = String(r.fornecedor_nome || r.nome_fornecedor || r.descricao || "").toLowerCase();
    return !search || nome.includes(search.toLowerCase());
  });

  const totalValor = filtered.reduce((s, r) => s + Number(r.valor ?? 0), 0);
  const totalPago = filtered.filter(r => {
    const sit = String(r.situacao || "").toLowerCase();
    return sit.includes("pago") || sit.includes("quitado");
      ...
      <div className="space-y-6 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Títulos a pagar via SSótica</p>
          </div>
        </div>
        ...
        {rows.length > 0 && (
          <div className="grid gap-4 sm:grid-cols-3">
            <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">Total a pagar</p><p className="text-2xl font-bold text-red-700">{fmtBRL(totalValor)}</p></CardContent></Card>
            ...
            <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">Total pago</p><p className="text-2xl font-bold text-emerald-700">{fmtBRL(totalPago)}</p></CardContent></Card>
          </div>
        )}
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <Input placeholder="Buscar fornecedor…" className="w-60" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filtered.length === 0 ? (
                      ...
                      <TableRow key={i}>
                        <TableCell className="font-medium">{getField(r, "fornecedor_nome", "nome_fornecedor", "descricao")}</TableCell>
                        <TableCell>{fmtDate(getField(r, "vencimento", "data_vencimento"))}</TableCell>
                        <TableCell>{getField(r, "numero_parcela", "parcela")}</TableCell>
                        <TableCell>{fmtBRL(Number(r.valor ?? 0))}</TableCell>
                        <TableCell>{situacaoBadge(getField(r, "situacao", "status"))}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
