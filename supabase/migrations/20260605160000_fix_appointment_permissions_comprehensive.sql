-- Corrige permissões de agendamentos: admin total, vendedor próprio, gerente da equipe.
-- RPCs com row_security = off para não serem bloqueadas pelo RLS do chamador.
...
BEGIN
  ...
  SELECT a.scheduled_by, COALESCE(a.is_reschedule_snapshot, false)
  INTO v_scheduled_by, v_is_snapshot
  ...
  IF v_is_snapshot THEN
    RETURN false;
  END IF;

  IF v_scheduled_by = auth.uid() THEN
    RETURN true;
  END IF;

  IF has_role(auth.uid(), 'gerente'::app_role) AND is_same_company(v_scheduled_by) THEN
    RETURN true;
  END IF;
